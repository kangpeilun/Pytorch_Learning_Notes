...
3. 获取形状：`tensor.size()`,也可以传入某个维度的索引，得到该维度对应的大小
...
6. 获取最大值：`tensor.max()`；tensor.min()获取最小值
...
3. CUDA中的tensor
   ...
   ```python
   #device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu") # 自动选择sh
